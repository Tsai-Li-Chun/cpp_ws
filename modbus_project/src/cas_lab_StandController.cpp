...
{
	adam5000_ctl.adam5000_readDI(static_cast<adam5000_DI>(ch));
...
{
	adam5000_ctl.adam5000_setDO(static_cast<adam5000_DO>(ch),data);
	return rc;
}
...
void cas_lab_StandController::check_action(void)
{
	...
	if( robot_info_brake_old != robot_info_brake )
	{
		...
		action_brake = true;
		std::cout << "robot_info_brake is change, action_brake = " << action_brake << std::endl;
	}
	/* check fixture */
	if( robot_info_fixture_old != robot_info_fixture )
	{
		...
		action_fixture = true;
		std::cout << "robot_info_fixture is change, action_fixture = " << action_fixture << std::endl;
	}
	/* check kinetrol */
	if( robot_info_kinetrol_old != robot_info_kinetrol )
	{
		...
		std::cout << "robot_info_kinetrol is change, action_kinetrol = " << action_kinetrol << std::endl;
	}
	/* check toolATC  */
	if( robot_info_toolATC_old != robot_info_toolATC )
	{
		...
		action_toolATC = true;
		std::cout << "robot_info_toolATC is change, action_toolATC = " << action_toolATC << std::endl;
	}
	/* check regulate  */
	if( armM5_info_regulate != robot_info_regulate_kg )
	{
		armM5_info_regulate = robot_info_regulate_kg;
		action_regulate = true;
		std::cout << "robot_info_regulate_kg is change, action_regulate = " << action_regulate << std::endl;
	}
}
void cas_lab_StandController::action_stand(void)
{
	...
	if( action_regulate )
	{
		...
		armM5_info_regulate = regulate_kg2bar * regulate_bar2bin;
		if(armM5_info_regulate<0) armM5_info_regulate=0;
		if(armM5_info_regulate>2048)armM5_info_regulate=2048;
	...
	deltaDRV_ctl.write_deltaDRV_DataBuffer(static_cast<int>(robot_adr::action_cmd),static_cast<uint16_t>(robot_action_invalid));
	std::cout << "robot command address clean." << std::endl;
}
